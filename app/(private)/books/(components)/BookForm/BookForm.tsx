...
import type { UseBookFormParams } from './hooks';

import { useBooksForm } from './hooks';
...
type BookFormProps = UseBookFormParams;

export const BookForm = (props: BookFormProps) => {
  const { form, functions, state } = useBooksForm(props);
    ...
    <form onSubmit={functions.onSubmit}>
      <Stack gap='lg'>
        ...
        <Button loading={state.loading} mt='xl' size='xl' type='submit' variant='light'>
          {props.action === 'add' ? 'Создать' : 'Изменить'}
        </Button>
      </Stack>
    </form>
  );
